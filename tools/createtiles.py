...
try:
    with Image.open(sys.argv[2]) as img, open(sys.argv[3],mode="w") as output_file:
            ...
            for hcount in range(horizontal_count):
                ...
                for y in range(tile_size):
                    for i in range(tile_size//ppb):
                        byte=0
                        for j in range(ppb):
                            val=img.getpixel((xoffset+i*ppb+j,yoffset+y))
                            byte=byte<<bpp
                            if bpp==1:
                                if val>0:
                                    byte|=1
        ...
        output_file.write("};\n");
        output_file.write("struct TileSet "+name.lower()+"_tiles = {")
        output_file.write(str(tile_count)+","+str(bpp)+","+str(buffer_size)+",data};\n")
except IOError:
